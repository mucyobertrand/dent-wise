  ...
  useEffect(() => {
    ...
    const handleError = (error: any) => {
      ...
      setMessages(prev => [...prev, { 
        content: `Connection error: ${error?.message || 'Unknown error occurred'}. Please try again.`, 
        role: "system" 
      }]);
    };
  ...
  const toggleCall = async () => {
    if (callActive) {
      ...
      try {
        ...
        console.log("Starting Vapi call with assistant ID:", process.env.NEXT_PUBLIC_VAPI_ASSISTANT_ID);
        console.log("Vapi API key available:", !!process.env.NEXT_PUBLIC_VAPI_API_KEY);
        ...
        setConnecting(false);
        // Add error message to messages for user feedback
        setMessages([{ 
          content: "Failed to connect to voice assistant. Please check your connection and try again.", 
          role: "system" 
        }]);
      }
    }
