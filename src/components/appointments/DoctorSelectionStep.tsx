...
function DoctorSelectionStep({
  ...
  console.log("Doctors data:", dentists);
                ...
                <div className="w-16 h-16 rounded-full bg-primary/10 flex items-center justify-center overflow-hidden">
                  {dentist.imageUrl ? (
                    <Image
                      ...
                      unoptimized={true}
                      onError={(e) => {
                        console.log("Image failed to load:", dentist.imageUrl);
                        const target = e.currentTarget;
                        const parent = target.parentElement;
                        if (parent) {
                          target.style.display = "none";
                          parent.innerHTML = '<div class="w-full h-full bg-gray-300 flex items-center justify-center text-gray-500 text-xs">No Image</div>';
                        }
                      }}
                    />
                  ) : (
